...
public class GravityCalculation{
	...
	static long gravConstantMicroEarthMass=16;
	GravityCalculation(){	
	}
	public static void computeGPlanets(CelestialBody[] planetSystem, int frequency){
		...
		for(int ii=0; ii<planetSystem.length; ii++){
			...
			for(int jj=0; jj<planetSystem.length; jj++){
				...
				if(distanceRkm!=0){
					accXkmByS+=gravConstantMicroEarthMass*planetSystem[jj].getMass()*(planetSystem[jj].getX()-planetSystem[ii].getX())/Math.pow(distanceRkm, 3);
					accYkmByS+=gravConstantMicroEarthMass*planetSystem[jj].getMass()*(planetSystem[jj].getY()-planetSystem[ii].getY())/Math.pow(distanceRkm, 3);
				}
			}
	...
	public static void computeGShip(CelestialBody[] planetSystem, int frequency, Ship rocket){
		...
		for(int jj=0; jj<planetSystem.length; jj++){
			...
			if(distanceRkm!=0){
				...
				accXkmByS+=gravConstantMicroEarthMass*planetSystem[jj].getMass()*(planetSystem[jj].getX())/Math.pow(distanceRkm, 3);
				accYkmByS+=gravConstantMicroEarthMass*planetSystem[jj].getMass()*(planetSystem[jj].getY())/Math.pow(distanceRkm, 3);
